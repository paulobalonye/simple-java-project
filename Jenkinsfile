pipeline {
  ...
  stages {
    ...
    stage ('Nexus Upload') {
      ...
      nexusVersion: 'nexus3',
      protocol: 'http',
      nexusUrl: 'http://3.141.140.73:8081',
      groupId: 'myGroupId',
    ...
    stage ('Slack Notification') {
     ...
     stage ('QA Deploy') {
      steps {
        ...
        deploy adapters: [tomcat9(credentialsId: '268c42f6-f2f5-488f-b2aa-f2374d229b2e', path: '', url: 'http://3.129.107.164:8080')], contextPath: null, war: '**/*.war'
        }
    }
    stage ('QA Approve') {
      steps {
